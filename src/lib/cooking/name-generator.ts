/**
 * Name Generator - Creates descriptive names for cooked foods
 *
 * @remarks
 * **Algorithm:**
...
export function generateFoodName(input: NameGeneratorInput): GeneratedName {
    ...
    const _primary = ingredients.reduce((a, b) => (a.tier > b.tier ? a : b));

    // Identify modifiers from ingredients
    const modifiers = identifyModifiers(ingredients, spice);

    // Select adjective based on modifiers
...
function selectAdjective(
    modifiers: string[],
    _cookingType: 'CAMPFIRE' | 'POT' | 'OVEN'
): { en: string; vi: string } {
