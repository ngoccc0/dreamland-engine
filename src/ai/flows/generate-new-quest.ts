...
let generateNewQuestFlow: any = null;

async function initGenerateNewQuestFlow() {
    if (generateNewQuestFlow) return;
    const ai = await getAi();
    generateNewQuestFlow = ai.defineFlow(
        {
...
export async function generateNewQuest(input: GenerateNewQuestInput): Promise<GenerateNewQuestOutput> {
    await initGenerateNewQuestFlow();
