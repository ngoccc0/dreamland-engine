...
export const RecipeIngredientSchema = z.object({
  name: z.string().describe("The unique name (key) of the ingredient item."),
...
export const RecipeResultSchema = z.object({
    name: z.string().describe("The unique name (key) of the crafted item."),
    quantity: z.number().int().min(1).describe("The quantity of the item produced."),
    emoji: z.string().describe("An emoji to represent the crafted item in the UI.")
});
...
export const RecipeSchema = z.object({
    ...
    description: MultilingualTextSchema.describe("A brief, multilingual description of what this recipe creates."),
