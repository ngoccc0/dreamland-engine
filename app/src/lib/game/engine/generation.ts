...
const selectEntities = <T extends { name?: TranslatableString | string; type?: string; data?: any; conditions?: any }>(
    ...
    for (const entity of shuffled) {
        ...
        const itemName = 'name' in entityData ? getTranslatedText(entityData.name, 'en') : entityData.type;
        const itemDef = allItemDefinitions[itemName];
        if (itemDef) {
            const tierMultiplier = Math.pow(0.75, itemDef.tier - 1); // Less punishing than 0.5
