...
Core architectural patterns used in Dreamland Engine. See **GUIDES_HOW_TO.md** for practical how-to guides.
...
```typescript
/**
 * Brief one-liner of what this function does.
 *
 * @remarks
 * Detailed explanation of the logic, algorithm, or decision.
 * **Use bold** for emphasis.
 * Use lists for multi-step algorithms:
 * 1. First step
 * 2. Second step
 * 3. Third step
 *
 ...
 *
 * @param paramName - Description and expected range
 * @returns Return value description and range
 *
 * @example
 * functionName(5) → 10 (because 5 * 2)
 */
```

### Key Rules
- ✅ Summary line < 100 chars
- ✅ @remarks for algorithm/logic (NOT inline comments)
- ✅ @param for inputs with expected ranges
- ✅ @returns for output meaning
- ✅ @example with concrete values
- ✅ NO inline comments: move to @remarks
- ✅ 100% coverage on exports

### Anti-Pattern (❌ DO NOT DO)
```typescript
// ❌ WRONG - Inline comments explaining logic
export function calculateDamage(attack: number, defense: number): number {
  // attack - defense = base damage  ← Move to @remarks
  // clamp to minimum of 1 ← Move to @remarks
  return Math.max(1, attack - defense);
}

// ✅ CORRECT - Logic in @remarks
/**
 ...
 *
 * **Logic:**
 * 1. Subtract defense from attack
 * 2. Enforce minimum of 1
 *
 * @param attack - Attacker stat
 * @param defense - Defender stat
 * @returns Base damage (minimum 1)
 */
export function calculateDamage(attack: number, defense: number): number {
  return Math.max(1, attack - defense);
