...
describe('applyTickEffects', () => {
  test('applies poison damage and decrements duration', () => {
    const stats = { hp: 10, stamina: 5, hunger: 50, statusEffects: [{ id: 'e1', type: 'poison', duration: 2, magnitude: 2, description: { en: 'poison' }, appliedTurn: 0 }] };
  ...
  test('hunger decay and starvation damage', () => {
    const stats = { hp: 5, hunger: 0.4, statusEffects: [] };
  ...
  test('effect expiry message when duration hits zero', () => {
    const stats = { hp: 20, hunger: 50, statusEffects: [{ id: 'e2', type: 'weakness', duration: 1, magnitude: 1, description: { en: 'weak' }, appliedTurn: 0 }] };
