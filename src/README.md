...
| Thuộc tính       | Kiểu dữ liệu                     | Bắt buộc? | Mô tả                                                               |
| ---------------- | -------------------------------- | --------- | ------------------------------------------------------------------- |
| `result`         | `{ name: string, quantity: number, emoji: string }` | **Có**    | Tên, số lượng và emoji của vật phẩm tạo ra.                          |
...
| `requiredTool`   | `string` (Item Name)             | Không     | Tên của công cụ cần có trong hành trang để thực hiện công thức.      |

**Ví dụ: Công thức chế tạo "Bó Đuốc"**
...
```
*Lưu ý: Tên vật phẩm (`name`) phải khớp chính xác với tên trong định nghĩa vật phẩm.*
  ...
  "swamp": [
    {
      "data": {
        ...
        "maxSatiation": 2,
        "loot": [
          { "name": "Nước Bùn", "chance": 0.5, "quantity": { "min": 1, "max": 2 } }
        ]
      },
      "conditions": {
        "chance": 0.2,
