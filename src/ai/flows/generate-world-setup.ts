...
import {z} from 'zod';
import type { Terrain, Skill, Structure, GeneratedItem, ItemDefinition } from '@/lib/game/types';
...
const AIGeneratedItemCreativeSchema = z.object({
  ...
  naturalSpawn: z.array(z.object({ biome: z.enum(allTerrains), chance: z.number().min(0).max(1) })).min(1).describe(`An array of one or more biomes where this item can be found. Choose from: ${allTerrains.join(', ')}.`),
});
...
4.  The 'naturalSpawn' must be an array of objects, each with a 'biome' from this list: ${allTerrains.join(', ')} and a 'chance'.
