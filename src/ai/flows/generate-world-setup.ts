...
const AIGeneratedItemCreativeSchema = z.object({
  ...
  naturalSpawn: z.array(z.object({ biome: z.enum(allTerrainsSchema), chance: z.number().min(0).max(1) })).min(1).describe(`An array of one or more biomes where this item can be found. Choose from: ${allTerrainsSchema.join(', ')}.`),
});
...
2.  For each item, you MUST define only 'name', 'description', 'category', and 'naturalSpawn'.
3.  The 'category' must be one of the allowed values: ${ItemCategorySchema.options.join(', ')}.
4.  The 'naturalSpawn' must be an array of objects, each with a 'biome' from this list: ${allTerrainsSchema.join(', ')} and a 'chance'.
...
const generateWorldSetupFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    const customItemCatalog: ItemDefinition[] = creativeItems.map(item => {
      const validBiomes = item.naturalSpawn.filter(b => allTerrainsSchema.includes(b.biome as Terrain));
        if (validBiomes.length === 0) {
            validBiomes.push({ biome: 'forest', chance: 0.1 }); // Add a fallback biome if the AI provides an invalid one
        }
        return {
            ...item,
            name: item.name,
    ...
    const finalOutput: GenerateWorldSetupOutput = {
        customItemCatalog: customItemCatalog as GeneratedItem[], // Cast to match output type
        customStructures,
