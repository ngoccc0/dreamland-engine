...
import { PlantPropertiesSchema } from './plant-properties';
...
export const CreatureDefinitionSchema = z.object({
  ...
  plantProperties: PlantPropertiesSchema.optional().describe("Special properties for plant creatures affecting vegetation density and reproduction."),

  /**
   * Optional natural spawn rules for this creature. This allows a creature definition
   * to declare where and how it may appear naturally in the world without requiring
   * templates to list it explicitly. Each entry can specify a `biome`, `chance` and
   * optional `conditions` object compatible with the spawn condition checks.
   */
  naturalSpawn: z.array(z.object({
      biome: z.string().optional(),
      chance: z.number().min(0).max(1).optional(),
      conditions: z.record(z.any()).optional()
  })).optional().describe("Optional natural spawn entries (biome/chance/conditions) used by the world generator."),
