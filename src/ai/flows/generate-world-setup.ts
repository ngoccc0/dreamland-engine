  ...
  async (input) => {
    ...
    if (input.userInput.toLowerCase().trim() === 'floptropica') {
        ...
        const hardcodedWorld: GenerateWorldSetupOutput = {
            customItemCatalog: floptropicaItems as any, // Cast to bypass type mismatch with schema's attribute definition
            customStructures: floptropicaStructures,
    ...
    const allTerrains: Terrain[] = ["forest", "grassland", "desert", "swamp", "mountain", "cave", "jungle", "volcanic", "tundra", "beach", "mesa", "mushroom_forest" ];
